        ...
        <ul class="nav {{ $horizontalTabs ? 'nav-tabs' : 'nav-stacked nav-pills'}}" role="tablist">
            @foreach ($crud->getTabs() as $k => $tab)
                <li role="presentation" class="{{$k == 0 ? 'active' : ''}}">
                    <a href="#tab_{{ str_slug($k, "") }}" aria-controls="tab_{{ str_slug($k, "") }}" role="tab" tab_name="{{ str_slug($k, "") }}" data-toggle="tab" class="tab_toggler">{{ $tab }}</a>
                </li>
            @endforeach
        </ul>
    </div>
...
<div class="tab-content box {{$horizontalTabs ? 'col-md-12' : 'col-md-9 m-t-10'}}">
    ...
    @foreach ($crud->getTabs() as $k => $tab)
    <div role="tabpanel" class="tab-pane{{$k == 0 ? ' active' : ''}}" id="tab_{{ str_slug($k, "") }}">
